...
function handle_ctrlc() {
    ...
    rm -f profile_source.tar.gz profile-startup-script.sh target.tar.gz
    
    if gcloud compute instances describe "$INSTANCE_NAME" --zone="$ZONE" &> /dev/null; then
        echo "Attempting to upload profiling results from VM..."
        gcloud compute ssh "$INSTANCE_NAME" --zone="$ZONE" --ssh-flag="-o ConnectTimeout=10" --command="cd /opt/tiny-llm && gcloud storage cp current_run.* $BUCKET_NAME/profiles/$RUN_ID/" 2>/dev/null || echo "⚠️ No profiles found or SSH not ready."
        ...
        echo ""
        echo "⬇️  To download your profiles locally, run:"
        echo "    mkdir -p profiles/$RUN_ID"
        echo "    gcloud storage cp $BUCKET_NAME/profiles/$RUN_ID/current_run.* profiles/$RUN_ID/"
    else
...
tar -czf profile_source.tar.gz src/ Cargo.toml Cargo.lock build.rs config.json tokenizer.json .agent/

echo "☁️  Step 3: Uploading assets to GCP Storage..."
gcloud storage cp profile_source.tar.gz "$BUCKET_NAME/run/"
...
cat << 'INNSCRIPT' > profile-startup-script.sh
#!/bin/bash
set -e
exec > >(tee -a /var/log/startup-training.log) 2>&1

echo "Starting headless TinyLLM Profiling..."
...
gcloud storage cp "$BUCKET_NAME/run/profile_source.tar.gz" .
tar -xzf profile_source.tar.gz
rm profile_source.tar.gz
...
echo "Running profiling loop..."
MAX_STEPS=25 bash .agent/skills/profiling/scripts/nsys_profile.sh ./target/release/train_benchmark current_run || true

echo "Uploading profiles back to bucket..."
gcloud storage cp current_run.* "$BUCKET_NAME/profiles/$RUN_ID/" || true
gcloud storage cp current_run_report.* "$BUCKET_NAME/profiles/$RUN_ID/" || true
...
sed -i "s|\[INJECT_BUCKET_NAME\]|$BUCKET_NAME|g" profile-startup-script.sh
sed -i "s|\[INJECT_INSTANCE_NAME\]|$INSTANCE_NAME|g" profile-startup-script.sh
sed -i "s|\[INJECT_ZONE\]|$ZONE|g" profile-startup-script.sh
sed -i "s|\[INJECT_RUN_ID\]|$RUN_ID|g" profile-startup-script.sh

echo "🌩️ Step 4: Provisioning VM..."
gcloud compute instances create $INSTANCE_NAME \
    --zone=$ZONE \
    --machine-type=a2-highgpu-1g \
    --network-interface=network-tier=PREMIUM,stack-type=IPV4_ONLY,subnet=ai-frontend-network \
    --metadata-from-file=startup-script=profile-startup-script.sh \
...
rm profile_source.tar.gz profile-startup-script.sh
